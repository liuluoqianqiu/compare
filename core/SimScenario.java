...
public class SimScenario implements Serializable {
	...
	//RSU标记
		private static String RSU_GROUP = "r";
...
//				hosts.add(host);
				...
				DTNHost host;//创建自定义的RSU实例和vehicle实例
				if(RSU_GROUP.equals(gid)) {
					host = new RSU(this.messageListeners, 
							this.movementListeners,	gid, interfaces, comBus, 
							mmProto, mRouterProto);
					RSU.AllRSUs.add((RSU)host);
				}else {
					host = new Vehicle(this.messageListeners, 
							this.movementListeners,	gid, interfaces, comBus, 
							mmProto, mRouterProto);
					Vehicle.AllVehicles.add((Vehicle)host);
				}
				hosts.add(host);
			}
		}
